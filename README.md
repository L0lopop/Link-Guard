...
**Знает мошеннические сайты в лицо.** База из семи публичных списков — около 3 млн адресов — собирается в этом репозитории каждую ночь и приезжает готовым файлом. Проверка идёт на устройстве: ни один адрес наружу не уходит.

**Ловит чужое имя с приставкой.** `sberbank-shop.ru`, `tinkoffshop.com`, `vtb-online.info` — ни опечатка, ни бренд в поддомене такое не видят, расстояние слишком большое.

**Считает репутацию доменных зон, а не ведёт список вручную.** В зоне `digital` мошеннических сайтов в 195 раз больше, чем известных, в `xyz` — в 90. При этом в `com` их больше всех по числу, но на фоне настоящих сайтов это капля, поэтому `com` молчит.

**Знает свежерегистрированные домены.** Адрес, заведённый меньше десяти дней назад и носящий чужое имя, — почти всегда мошенничество, и в публичные списки он попадёт лишь через несколько дней.
