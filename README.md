...
**Знает мошеннические сайты в лицо.** База из семи публичных списков — больше 2,8 млн адресов — собирается в этом репозитории каждую ночь и приезжает готовым файлом. Проверка идёт на устройстве: ни один адрес наружу не уходит. Обновляется сама, ночью, чтобы не тратить трафик днём.
...
- **база мошеннических сайтов** — раз в сутки ночью скачивает готовый файл из релиза этого репозитория; какие адреса вы проверяете, при этом не сообщается никому;
